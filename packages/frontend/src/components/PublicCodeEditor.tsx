...
import { usePublicFileContent } from '../hooks/usePublicApi';
...
export default function PublicCodeEditor({
  token,
  filePath,
  issues,
  onSelectIssue,
  ...
  highlightLines,
  shareToken,
}: PublicCodeEditorProps) {
  const { data: content, isLoading } = usePublicFileContent(token, filePath);
      ...
      <div className="flex-1 min-h-0 relative overflow-hidden">
        ...
        {hoveredIssue && (
          <IssueHoverCard
            ...
            shareUrl={hoveredIssue.lineStart && shareToken ? (() => {
              ...
              return `${window.location.origin}/share/${shareToken}/code/${encodeURIComponent(filePath)}?L=${lineParam}`;
            })() : undefined}
          />
        )}
      </div>
